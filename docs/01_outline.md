...
Expose weights through simple strategy names on a dense preference grid, so
students can dial in a specific trade-off (not just a couple of presets):

```python
# scalarized_<w1>_<w2>, w1 in {0.05, 0.10, ..., 0.95}, w2 = 1 - w1
"scalarized_0.05_0.95"  # ... favors objective 2
"scalarized_0.5_0.5"    # balanced
"scalarized_0.95_0.05"  # ... favors objective 1
```

Generated by `acquisitions.scalarized_grid(step=0.05)` and included in
`STRATEGY_NAMES`; off-grid weights (e.g. `scalarized_0.73_0.27`) also build.
...
3. `"scalarized_<w1>_<w2>"`: fixed-weight scalarization on a dense grid
   (`w1` = 0.05, 0.10, ..., 0.95; `w2 = 1 - w1`), spanning balanced
   (`scalarized_0.5_0.5`) through strongly objective-1- or -2-favoring.
4. `"random"`: random unevaluated sequence baseline.
5. Optional: `"uncertainty"`: exploration-heavy candidate.
...
"random"
"uncertainty"
# plus the dense scalarized grid from scalarized_grid(step=0.05):
"scalarized_0.05_0.95", "scalarized_0.1_0.9", ..., "scalarized_0.95_0.05"
```
...
| 14 | `docs/15_notebook04_extensions.md` | `notebooks/04_optional_extensions.ipynb` | **DONE** |
